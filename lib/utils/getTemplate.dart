...
String getTemplate(Notice notice) {
  ...
  "of the control statement as sales of goods to Helena Pospisilova (VAT ID IDDDD}) whereas "
  "they were reported by the purchaser in section B.2. of the control statement as goods purchased from "
  "${notice.companyName}:"
  "<br><br>"
  "<ul>${getInvoiceTemplate(notice.invoices)}</ul>"
  "<br>"
  "Moreover, the authorities identified that the following invoice was reported by ${notice.companyName} in section "
  "B.2. of the control statement as goods/services purchased from its supplier Partia RP s.r.o. "
  "(VAT ID CZXXXXXXXXZ) whereas it was not reported by the supplier in section A.4. of the control "
  ...
  "<br><br>"
  "The deadline for submission of the subsequent control statement regarding the above is on Tuesday, "
  "19 June 2022 (17 calendar days after the official letter was received). Therefore, the action from your "
...
String getInvoiceTemplate(List<Invoice> invoices) {
  ...
  for(final Invoice invoice in invoices) {
    ...
    for(final Vat vat in invoice.vats) {
      if(vat.vatAmount != 0.0 && vat.vatBase != 0.0) {
        final int vatRate = (vat.vatAmount / vat.vatBase * 100).toInt();
        invoiceTemplate += ", tax base (for VAT rate $vatRate%) in CZK ${NumberFormat.currency(symbol: "").format(vat.vatBase)}, VAT amount ($vatRate%) in CZK ${NumberFormat.currency(symbol: "").format(vat.vatAmount)}";
      }
    }
    invoiceTemplate += "</li>";
  }
